...
if (!file_exists(WB_PATH . '/modules/' . basename(dirname(__FILE__)) . '/languages/' . LANGUAGE . '.php')) {
  require_once (WB_PATH . '/modules/' . basename(dirname(__FILE__)) . '/languages/EN.php'); // EN
                                                                                      // is
                                                                                      // default!
}
else {
...
class facebook_page_tool {
  ...
  protected function getRecord() {
    ...
    if ($query->numRows() < 1) {
      ...
      // set field ID to -1 to indicate that record does not exists
      $this->setFieldID(-1);
      // set empty string
      $this->setFieldAppID('');
      // set empty string
      $this->setFieldSecret('');
      // set default path to index.php
      $this->setFieldPath('/modules/fb_page_tool/fb/1/index.php');
      // set empty string
      $this->setFieldText('');
      return true;
    }
    ...
    // MYSQL_ASSOC returns a record with field names as index
    $record = $query->fetchRow(MYSQL_ASSOC);
    // set field ID
    $this->setFieldID($record['fb_pt_id']);
    // set field AppID
  ...
  protected function setRecord($record = array()) {
    ...
    if ($this->getFieldID() < 1) {
      // insert a new record
      $SQL = "INSERT INTO " . TABLE_PREFIX . "mod_fb_page_tool (fb_pt_id, fb_pt_app_id, fb_pt_app_secret, fb_pt_page_path, fb_pt_page_text) " . "VALUES (1, '" . $this->getFieldAppID() . "', '" . $this->getFieldSecret() . "', '" . $this->getFieldPath() . "', '" . $this->getFieldText() . "')";
      ...
      $SQL = "UPDATE " . TABLE_PREFIX . "mod_fb_page_tool SET " . "fb_pt_app_id='" . $this->getFieldAppID() . "', " . "fb_pt_app_secret='" . $this->getFieldSecret() . "', " . "fb_pt_page_path='" . $this->getFieldPath() . "', " . "fb_pt_page_text='" . $this->getFieldText() . "' WHERE fb_pt_id='1'";
      // exec the SQL statement
      if (!$database->query($SQL)) {
        // on error save the database prompt and return false
        $this->setError(sprintf('[%s - %s] %s', __METHOD__, __LINE__, $database->get_error()));
  ...
  protected function getTemplate($template, $template_data) {
    ...
    try {
      ...
      $this->setError(sprintf('[%s - %s] %s', __METHOD__, __LINE__, $this->lang(sprintf('error_executing_template', $template, $e->getMessage()))));
      return false;
    }
    return $result;
  } // getTemplate()
